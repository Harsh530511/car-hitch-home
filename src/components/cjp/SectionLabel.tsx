interface Props {
  label: string;
  title: string;
  emphasis: string;
  description?: string;
  as?: "h1" | "h2";
}

const SectionLabel = ({ label, title, emphasis, description, as = "h2" }: Props) => {
  const Heading = as;
  return (
    <div className="border-b border-ink">
      <div className="max-w-7xl mx-auto px-4 sm:px-8 py-12 sm:py-16">
        <p className="font-condensed text-xs text-ink/60 mb-4">{label}</p>
        <Heading className="font-display text-5xl sm:text-7xl lg:text-8xl leading-[0.9] tracking-tight">
          {title} <span className="font-italic-serif text-ink/80">{emphasis}</span>
        </Heading>
        {description && (
          <p className="mt-6 max-w-2xl text-base sm:text-lg text-ink/70 leading-relaxed">{description}</p>
        )}
      </div>
    </div>
  );
};

export default SectionLabel;
